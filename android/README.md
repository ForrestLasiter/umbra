...
android/
  app/src/main/java/com/forrestlasiter/umbra/
    ...
    vpn/       UmbraVpnService, TunnelController              (DNS-sinkhole svc + mode select)
...
`TunnelController` picks the datapath per posture: **WireGuard** for tunnel
postures, the **DNS sinkhole** for telemetry-only postures (`home`).
